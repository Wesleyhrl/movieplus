    ...
    return (
        <div className="Details d-flex flex-column ms-auto me-auto">
            ...
            <div className="backdrop d-flex justify-content-center align-items-center">
                <img className="img-fluid" src={`https://image.tmdb.org/t/p/original/${streaming.backdrop_path}`} alt={streaming.title} />
            ...
            <div>
                <div className="movie-info d-flex flex-column flex-md-row justify-content-center flex-wrap align-items-center">
                    <span>{new Date(streaming.release_date || streaming.first_air_date).toLocaleDateString()}</span>
                    <div className="text-wrap">
                        {streaming.genres.map((genres, i, array) => {
                            return (
                                <span key={genres.id}>{genres.name}{(i + 1 < array.length) ? ", " : ""}</span>
                            )
                        })}
                    </div>
                    {runtime.length ? (<span>{`${type === "tv" ? runtime.join("/") : runtime}`}{type === "tv" && "Min"}</span>) : "N/A"}
